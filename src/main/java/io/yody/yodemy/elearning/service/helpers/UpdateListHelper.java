...
public class UpdateListHelper {
    ...
    public static <PARENT, BO, REQUEST> void updateListBO(
        ...
        if (Objects.isNull(requests)) return;
        Map<Long, REQUEST> requestMap = requests.stream().collect(Collectors.toMap(getRequestIdFunction, request -> request));
        if (ObjectUtils.isEmpty(bos)) return;
