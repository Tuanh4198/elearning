...
export default () => next => action => {
  ...
  if (isFulfilledAction(action) && payload && payload.headers) {
    ...
    if (alert) {
      ...
      notiSuccess(translate(alert, { param: alertParam }));
    }
  }
  ...
  if (isRejectedAction(action) && error && error.isAxiosError) {
    if (error.response) {
          ...
          case 400: {
            ...
            if (errorHeader) {
              ...
              for (const fieldError of fieldErrors) {
                ...
                const fieldName = translate(`yodemyApp.${fieldError.objectName}.${convertedField}`);
                addErrorAlert(`Error on field "${fieldName}"`, `error.${fieldError.message}`, { fieldName });
              }
            } else if (typeof data === 'string' && data !== '') {
              addErrorAlert(data);
            } else {
              notiError(data?.message || data?.error || data?.title || 'Unknown error!');
            }
            break;
          }
          case 404:
      ...
      notiError(error.message || 'Unknown error!');
    }
  } else if (error) {
    notiError(error.message || 'Unknown error!');
  }
