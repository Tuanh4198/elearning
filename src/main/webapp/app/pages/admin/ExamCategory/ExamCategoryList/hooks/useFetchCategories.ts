...
export const useFetchCategories = (filter: IFilterParams) => {
  ...
  const fetchCategories = async () => {
    const params = {
      ...
      type: CategoryTypeEnum.EXAM,
    };
    return await axios.get<ICategory[]>(`/api/exam-employees/categories`, { params });
  };

  const { isLoading, isFetching, data, refetch } = useQuery({
    queryKey: ['useFetchExamCategoriesAdmin', filter.page, size, filter.search],
